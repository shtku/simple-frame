...
(function(window, document, Math, undefined) {
	...
	/*
	 * CustomEvent polyfill for IE9 By MDN
	 * https://developer.mozilla.org/en-US/docs/Web/API/CustomEvent MIT LICENSE
	 */
	...
	/**
	 * Optiscroll, use this to create instances ``` var scrolltime = new
	 * Optiscroll(element); ```
	 */
	...
	Optiscroll.Instance.prototype = {
		...
		init : function() {
			var me = this, settings = me.settings, shouldCreateScrollbars = false;

			me.scrollEl = Utils.createWrapper(me.element, settings.classPrefix
					+ 'content');
			...
			if (G.nativeScrollbarSize || settings.forceScrollbars) {
				shouldCreateScrollbars = Utils
						.hideNativeScrollbars(me.scrollEl);
			}
		...
		bind : function() {
			var me = this, listeners = me.listeners = {}, scrollEl = me.scrollEl;
			...
			if (G.isTouch) {
				listeners.touchstart = function(ev) {
					Events.touchstart(ev, me);
				};
				listeners.touchend = function(ev) {
					Events.touchend(ev, me);
				};
			}
		...
		update : function() {
			var me = this, oldcH = me.cache.clientH, scrollEl = me.scrollEl, cache = me.cache, sH = scrollEl.scrollHeight, cH = scrollEl.clientHeight, sW = scrollEl.scrollWidth, cW = scrollEl.clientWidth;
			...
			if (sH !== cache.scrollH || cH !== cache.clientH
				...
				if (oldcH !== undefined) {
					...
					if (sH === 0 && cH === 0
							&& !Utils.containsNode(document.body, me.element)) {
						me.destroy();
						return false;
					}
		...
		scrollTo : function(destX, destY, duration) {
			var me = this, cache = me.cache, startX, startY, endX, endY;
		...
		scrollIntoView : function(elem, duration, delta) {
			var me = this, scrollEl = me.scrollEl, eDim, sDim, leftEdge, topEdge, rightEdge, bottomEdge, offsetX, offsetY, startX, startY, endX, endY;
			...
			rightEdge = offsetX + eDim.width - me.cache.clientW
					+ (delta.right || 0);
			bottomEdge = offsetY + eDim.height - me.cache.clientH
					+ (delta.bottom || 0);
		...
		animateScroll : function(startX, endX, startY, endY, duration) {
			var me = this, scrollEl = me.scrollEl, startTime = Date.now();
			...
			if (isNaN(duration)) { // undefined or auto
				...
				duration = Math.pow(Math.max(Math.abs(endX - startX), Math
						.abs(endY - startY)), 0.54) * 15;
			}

			(function animate() {
				var time = Math.min(1, ((Date.now() - startTime) / duration)), easedTime = Utils
						.easingFunction(time);

				if (endY !== startY) {
					scrollEl.scrollTop = ~~(easedTime * (endY - startY))
							+ startY;
				}
				if (endX !== startX) {
					scrollEl.scrollLeft = ~~(easedTime * (endX - startX))
							+ startX;
				}

				me.scrollAnimation = time < 1 ? window
						.requestAnimationFrame(animate) : null;
			}());
		},

		destroy : function() {
			var me = this, element = me.element, scrollEl = me.scrollEl, listeners = me.listeners, index = G.instances
					.indexOf(me), child;
		...
		fireCustomEvent : function(eventName) {
			var me = this, cache = me.cache, sH = cache.scrollH, sW = cache.scrollW, eventData;
			...
			me.element.dispatchEvent(new CustomEvent(eventName, {
				detail : eventData
			}));
		},
	...
	var Events = {
		...
		wheel : function(ev, me) {
			var cache = me.cache, cacheV = cache.v, cacheH = cache.h, preventScroll = me.settings.preventParentScroll;

			window.cancelAnimationFrame(me.scrollAnimation);

			if (preventScroll && cacheV.enabled && cacheV.percent % 100 === 0) {
				me.scrollEl.scrollTop = cacheV.percent ? (cache.scrollH
						- cache.clientH - 1) : 1;
			}
			if (preventScroll && cacheH.enabled && cacheH.percent % 100 === 0) {
				me.scrollEl.scrollLeft = cacheH.percent ? (cache.scrollW
						- cache.clientW - 1) : 1;
			}
		},
	...
	var Scrollbar = function(which, instance) {
		...
		var isVertical = (which === 'v'), parentEl = instance.element, scrollEl = instance.scrollEl, settings = instance.settings, cache = instance.cache, scrollbarCache = cache[which] = {},

		sizeProp = isVertical ? 'H' : 'W', clientSize = 'client' + sizeProp, scrollSize = 'scroll'
				+ sizeProp, scrollProp = isVertical ? 'scrollTop'
				: 'scrollLeft', evNames = isVertical ? [ 'top', 'bottom' ] : [
				'left', 'right' ],

		enabled = false, scrollbarEl = null, trackEl = null;
		...
		var events = {
			...
			dragMove : function(ev) {
				var evData = ev.touches ? ev.touches[0] : ev, delta, deltaRatio;

				ev.preventDefault();
				delta = isVertical ? evData.pageY - events.dragData.y
						: evData.pageX - events.dragData.x;
				deltaRatio = delta / cache[clientSize];

				scrollEl[scrollProp] = events.dragData.scroll + deltaRatio
						* cache[scrollSize];
			},
			...
			bind : function(on) {
				var method = (on ? 'add' : 'remove') + 'EventListener', type = G.isTouch ? [
						'touchmove', 'touchend' ]
						: [ 'mousemove', 'mouseup' ];
		...
		return {
			...
			update : function() {
				var me = this, newSize, oldSize, newDim, newRelPos, deltaPos;
				...
				deltaPos = Math.abs(newDim.position
						- (scrollbarCache.position || 0))
						* cache[clientSize];
			...
			style : function(newRelPos, deltaPos, newSize, oldSize) {
				if (newSize !== oldSize) {
					trackEl.style[isVertical ? 'height' : 'width'] = newSize
							* 100 + '%';
				}
				trackEl.style[G.cssTransform] = 'translate('
						+ (isVertical ? '0%,' + newRelPos + '%' : newRelPos
								+ '%' + ',0%') + ')';
			},

			calc : function() {
				var position = scrollEl[scrollProp], viewS = cache[clientSize], scrollS = cache[scrollSize], sizeRatio = viewS
						/ scrollS, sizeDiff = scrollS - viewS, positionRatio, percent;
			...
			fireEdgeEv : function() {
				...
				if (scrollbarCache.was !== percent && percent % 100 === 0) {
					...
					instance.fireCustomEvent('scrollreach'
							+ evNames[percent / 100]);
				}
	...
	var Utils = {
		...
		hideNativeScrollbars : function(scrollEl) {
			var size = G.nativeScrollbarSize, scrollElStyle = scrollEl.style;
			if (size === 0) {
				// hide Webkit/touch scrollbars
				var time = Date.now();
				scrollEl.setAttribute('data-scroll', time);
				return Utils.addCssRule('[data-scroll="' + time
						+ '"]::-webkit-scrollbar',
		...
		addCssRule : function(selector, rules) {
			...
			if (!styleSheet) {
				...
				styleSheet.appendChild(document.createTextNode('')); // WebKit
																		// hack
		...
		createWrapper : function(element, className) {
			var wrapper = document.createElement('div'), child;
		...
		containsNode : function(parent, node) {
			return parent.contains ? parent != node && parent.contains(node)
	...
	function getScrollbarWidth() {
		var htmlEl = document.documentElement, outerEl, innerEl, width = 0;
	...
	function cssTest(prop) {
		var ucProp = prop.charAt(0).toUpperCase() + prop.slice(1), el = document
				.createElement('test'), props = (prop + ' '
				+ [ 'Webkit', 'Moz', 'O', 'ms' ].join(ucProp + ' ') + ucProp)
				.split(' ');
	...
	function toggleClass(el, value, bool) {
		var classes = el.className.split(/\s+/), index = classes.indexOf(value);
	...
	function _throttle(fn, threshhold) {
		var last, deferTimer;
		return function() {
			var context = this, now = Date.now(), args = arguments;
